...
export const Dropzone: FC<Props> = ({ file, status, error, onFileSelect, onClear }) => {
    ...
    const handleFileSelect = (selectedFile: File) => {
        if (!isCsvFile(selectedFile)) {
            setValidationError('Пожалуйста, загрузите CSV файл');
            return;
        }
    ...
    const renderContent = () => {
        if (isProcessing) {
            return (
                <div className={styles.fileProcessing} data-testid="file-processing">
                    <Loader />
                </div>
            );
        }
    ...
    return (
        ...
        >
            ...
            {validationError && (
                <Typography size="l" color="error" data-testid="csv-error">
                    {validationError}
                </Typography>
            )}
            {isProcessing && !validationError && (
                <Typography size="l" data-testid="processing-status">
                    Обработка
                </Typography>
            )}
            {error && !validationError && (
                <Typography size="l" color="error" data-testid="error-message">
                    {error}
                </Typography>
            )}
        </div>
